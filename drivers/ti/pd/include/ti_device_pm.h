 ...
 * @enable: True to enable the device, false to power down the device.
 *
 * Uses power-on-enabled host semantics (TI_DEV_POWER_ON_ENABLED_HOST_IDX)
 * for firmware-managed devices like CPU cores. Follows the same retention
 * sequencing flow as ti_set_device_handler.
 */
