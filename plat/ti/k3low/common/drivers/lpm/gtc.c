...
int32_t k3low_lpm_sleep_suspend_gtc(void)
{
	...
	k3low_lpm_rtc_read_time(&gtc.suspend);
...
int32_t k3low_lpm_resume_gtc(void)
{
	...
	k3low_lpm_rtc_read_time(&gtc.resume);

	suspend_start_time = (uint64_t)(((uint64_t)gtc.suspend.sec_hi << 32) +
					gtc.suspend.sec_lo);
	resume_time = (uint64_t)(((uint64_t)gtc.resume.sec_hi << 32) +
				 gtc.resume.sec_lo);
	delta = (resume_time - suspend_start_time) * gtc.fid0;
	...
	/* enable GTC */
	mmio_write_32(K3_GTC_CFG1_BASE + GTC_CFG1_CNTCR,
		      gtc.hdbg | GTC_CFG1_CNTCR_EN);
