...
#define WKUP_CTRL_PMCTRL_IO_0_WRITE_MASK \
	(WKUP_CTRL_PMCTRL_IO_0_ISOCLK_OVRD	\
...
__wkupsramdata uint32_t num_main_plls_save_rstr = 3U;
...
bool k3low_lpm_check_can_io_latch(void)
{
	return ((mmio_read_32(WKUP_CTRL_MMR_SEC_5_BASE +
		 ...
		 CANUART_WAKE_OFF_MODE_STAT1_ENABLED) != 0U);
}
...
__wkupsramfunc static int32_t save_and_disable_usb_lpsc(void)
{
	...
	usb0_state = psc_raw_lpsc_get_state(K3_MAIN_PSC_BASE,
					    LPSC_MAIN_GP_USB0);
	...
	if (ret == 0) {
		usb1_state = psc_raw_lpsc_get_state(K3_MAIN_PSC_BASE,
						    LPSC_MAIN_GP_USB1);
		k3low_psc_raw_lpsc_set_state(K3_MAIN_PSC_BASE,
					     LPSC_MAIN_GP_USB1,
					     MDCTL_STATE_DISABLE, false);
...
__wkupsramsuspendentry __used void k3low_lpm_stub_entry(uint32_t mode)
{
...
int32_t k3low_lpm_set_io_isolation(bool enable)
{
	...
	if (enable) {
		...
		/* Wait for IO isolation status to be set on PMCTRL_IO_0 */
		do {
			ret = -ETIMEDOUT;
			reg = mmio_read_32(WKUP_CTRL_MMR_SEC_5_BASE +
					   WKUP_CTRL_PMCTRL_IO_0);
		...
		/* Wait for IO isolation status to be set on PMCTRL_IO_1 */
		do {
			ret = -ETIMEDOUT;
			reg = mmio_read_32(WKUP_CTRL_MMR_SEC_5_BASE +
					   WKUP_CTRL_PMCTRL_IO_1);
