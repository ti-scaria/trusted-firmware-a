 ...
 */
#define PSC_TRANSITION_TIMEOUT		100000U

__wkupsramfunc int32_t k3low_psc_raw_pd_wait(uint64_t psc_base, uint8_t pd)
{
	int32_t ret = 0;
	uint32_t i = PSC_TRANSITION_TIMEOUT;

	while (((mmio_read_32((uintptr_t)psc_base + PSC_PTSTAT) &
		 BIT(pd)) != 0U) && (i != 0U)) {
		k3low_lpm_delay_1us();
		i--;
	}
...
__wkupsramfunc void k3low_psc_raw_pd_initiate(uint64_t psc_base, uint8_t pd)
{
	mmio_write_32((uintptr_t)psc_base + PSC_PTCMD, BIT(pd));
}

__wkupsramfunc void k3low_psc_raw_pd_set_state(uint64_t psc_base, uint8_t pd,
					       uint32_t state, bool force)
{
	uint32_t pdctl = mmio_read_32((uintptr_t)psc_base + PSC_PDCTL(pd));
	...
	mmio_write_32((uintptr_t)psc_base + PSC_PDCTL(pd), pdctl);
}

__wkupsramfunc void k3low_psc_raw_lpsc_set_state(uint64_t psc_base, uint8_t lpsc,
						 uint32_t state, bool force)
{
	uint32_t mdctl = mmio_read_32((uintptr_t)psc_base + PSC_MDCTL(lpsc));
	...
	mmio_write_32((uintptr_t)psc_base + PSC_MDCTL(lpsc), mdctl);
}
...
__wkupsramfunc uint8_t psc_raw_lpsc_get_state(uint64_t psc_base, uint8_t lpsc)
{
	return (uint8_t)(MDSTAT_STATE_MASK &
			 mmio_read_32((uintptr_t)psc_base +
				      PSC_MDSTAT(lpsc)));
}
